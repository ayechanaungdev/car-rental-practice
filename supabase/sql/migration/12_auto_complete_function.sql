...
BEGIN
  ...
  LOOP
    ...
    -- 3. Free up all cars involved
    UPDATE cars SET status = 'available'
    WHERE id IN (
      SELECT car_id FROM bookings WHERE id IN (SELECT json_array_elements_text(v_owner.booking_ids)::uuid)
    );

    -- 4. Free up all drivers involved
